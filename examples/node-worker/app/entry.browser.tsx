...
async function callServer(id: string, args: any[]) {
  const submitEvent = args[0];
  if (!submitEvent || !submitEvent.target) {
    console.error(submitEvent);
    throw new Error("Server actions only support form submissions");
  }

  let url = new URL(window.location.href);
  url.searchParams.set("_rsc_action", id);

  const submissionInfo = getFormSubmissionInfo(
    submitEvent.target,
    url.pathname,
    {}
  );

  url = submissionInfo.url;
  url.searchParams.set("_rsc", "");
  url.searchParams.set("_rsc_action", id);

  const abortController = new AbortController();
  const response = fetch(url, {
    method: submissionInfo.method,
    body: submissionInfo.formData,
    headers: {
      "Content-Type": submissionInfo.encType,
    },
  ...
  const rscChunk = ReactDOMRSC.createFromFetch(response, {
    callServer,
  });
  const toAbort = lastAbortController;
